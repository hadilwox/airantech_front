...
const form = reactive<StudentRegisterPayload>({
  email: '',
  password: '',
...
const errors = reactive<Partial<Record<keyof StudentRegisterPayload, string>>>({})
const generalError = ref('')
const loading = ref(false)

async function handleSubmit() {
  Object.keys(errors).forEach((key) => delete errors[key as keyof StudentRegisterPayload])
  generalError.value = ''
  loading.value = true
  ...
  try {
    ...
    if (error instanceof ApiError && error.status === 422 && error.errors) {
      for (const field of Object.keys(error.errors)) {
        errors[field as keyof StudentRegisterPayload] = error.fieldError(field)
      }
...
<template>
  <AuthLayout>
    <Card>
      <h2 class="mb-6 text-lg font-semibold text-foreground">ثبت‌نام دانشجو</h2>
      ...
      <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
        ...
        <Button type="submit" :loading="loading" class="mt-2 w-full">ثبت‌نام</Button>
      </form>
